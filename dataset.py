...
def Dataset(set, norm = False, coef_norm = None, crop = None, sample = None, n_boot = int(5e5), surf_ratio = .1):
    ...
    for k, s in enumerate(tqdm(set, desc='loading dataset')):
        # Get the 3D mesh, add the signed distance function and slice it to return in 2D
        internal = pv.read(osp.join('E:/AirfRANS_Data/AirfRANS-main/Dataset', s, s + '_internal.vtu'))
        aerofoil = pv.read(osp.join('E:/AirfRANS_Data/AirfRANS-main/Dataset', s, s + '_aerofoil.vtp'))
        ...
        if sample is not None:
            ...
            y_surfs = [surf[i] for i in range(len(surf)) if surf[i] == True]
            # print(len(y_surfs), '/', len(surf))
            # quit()
